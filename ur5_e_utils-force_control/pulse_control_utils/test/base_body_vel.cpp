...
int main(int argc, char** argv)
{
  ...
  ros::init(argc, argv, "body_vel_test_node");
  ros::NodeHandle nh("~");
  ...
  {
    ROS_WARN_STREAM("Could not find parameter 'end_effector_vel', using default value...");
    ee_vel = 0.01;
  }
  ...
  {
    ROS_WARN_STREAM("Could not find parameter 'num_steps', using default value...");
    num_steps = 250;
  }
  ...
  {
    ROS_WARN_STREAM("Could not find parameter 'joint_omega_max', using default value...");
    joint_omega_max = 0.1;
  }
  ...
  */
  ...
  {
    u.setBodyCartesianVelocity(v);

    ROS_INFO_STREAM(
      "[" << (i0 + 1) << "/" << num_steps << "] Setting body velocity of frame " << ee_name << " to " << std::endl
          << "[" << v(0) << ", " << v(1) << ", " << v(2) << ", " << v(3) << ", " << v(4) << ", " << v(5)
          << "] <m/rad>/s...");
    ROS_INFO_STREAM("[" << (i0 + 1) << "/" << num_steps << "] Sending joint vector goal...");
    u.sendGoalAndWait();
    // ros::Duration(0.5).sleep();
  }
  ...
  Eigen::Isometry3d p_ref;
  p_ref.setIdentity();
  // p_ref.rotate(Eigen::Vector3d(v(3), v(4), v(5)) * num_steps * delta_t);
  p_ref.translate(Eigen::Vector3d(v(0), v(1), v(2)) * num_steps * delta_t);
  ROS_INFO_STREAM("Expected difference:");
  printTransform(p_ref);

  // Compute and print transform between before and after robot movement.
  Eigen::Isometry3d p_t1 = fwd_kin_before.inverse() * fwd_kin_after;
  ROS_INFO_STREAM("Observed difference:");
  printTransform(p_t1);

  Eigen::Vector3d t_err = p_ref.translation() - p_t1.translation();
