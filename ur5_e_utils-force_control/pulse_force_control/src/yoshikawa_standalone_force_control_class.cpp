#include "pulse_force_control/yoshikawa_standalone_force_control_class.hpp"
 ...
 */
pfc::YoshikawaStandaloneForceControlClass::YoshikawaStandaloneForceControlClass(
    ...
    nh, base_frame_name, probe_frame_name, tool_frame_name)
{
  ...
  ROS_DEBUG(
    "Initializing timer for YoshikawaStandaloneForceControlClass with period "
    "%.2f ms...",
    1e3 * this->fsm_period);
  this->fsm_timer = this->nh.createTimer(
    ros::Duration(this->fsm_period),
    &YoshikawaStandaloneForceControlClass::forceControlLoopTimerCallback, this);
}

/*
 * \brief Callback function for control loop timer
 */
void pfc::YoshikawaStandaloneForceControlClass::forceControlLoopTimerCallback(
  const ros::TimerEvent& e)
{
  ROS_DEBUG(
    "YoshikawaStandaloneForceControlClass::forceControlLoopTimerCallback");
  ...
  // updated tool frame.
  pfc::YoshikawaForceControlClass::forceControlLoopTimerCallback(e);
