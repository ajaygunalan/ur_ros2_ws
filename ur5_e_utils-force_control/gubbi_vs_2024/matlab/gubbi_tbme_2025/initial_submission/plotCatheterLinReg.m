function plotCatheterLinReg(sys_ab_struct, sys_c_struct, input_time_vec, output_img_file_base_name, varargin)
	p = inputParser();
	...
	tf_base_lin_reg = computeLinRegTransform(sys_ab_struct.tf_trans(robot_id_vec, :)');

	robot_pos_base = [sys_ab_struct.tf_trans(robot_id_vec, :), ones(length(robot_id_vec), 1)];
	robot_pos_lin_reg = tf_base_lin_reg * robot_pos_base';
	robot_pos_lin_reg = robot_pos_lin_reg(1:3, :)';

	sys_ab_pos_base = [sys_ab_struct.pt_src_base_link(sys_ab_id_vec, :), ones(length(sys_ab_id_vec), 1)];
	sys_ab_pos_lin_reg = tf_base_lin_reg * sys_ab_pos_base';
	sys_ab_pos_lin_reg = sys_ab_pos_lin_reg(1:3, :)';

	sys_c_pos_base = [sys_c_struct.pt_src_base_link(sys_c_id_vec, :), ones(length(sys_c_id_vec), 1)];
	sys_c_pos_lin_reg = tf_base_lin_reg * sys_c_pos_base';
	sys_c_pos_lin_reg = sys_c_pos_lin_reg(1:3, :)';
	...
	scatter(1e3 .* robot_pos_lin_reg(:, 1), 1e3 .* robot_pos_lin_reg(:, 2), '.', 'DisplayName', 'Transducer');
	hold on
	scatter(1e3 .* sys_ab_pos_lin_reg(:, 1), 1e3 .* sys_ab_pos_lin_reg(:, 2), '.', 'DisplayName', legend_str);
	scatter(1e3 .* sys_c_pos_lin_reg(:, 1), 1e3 .* sys_c_pos_lin_reg(:, 2), '.', 'DisplayName', 'WaveSegNet-2');
	hold off
	legend();
	...
	output_img_file_name = fullfile(output_img_dir_name, ['raw_cath_lin_reg', output_img_file_base_name]);
	fprintf(['Saving file ''', output_img_file_name, '''...\n']);
	saveas(gcf, output_img_file_name);
end
