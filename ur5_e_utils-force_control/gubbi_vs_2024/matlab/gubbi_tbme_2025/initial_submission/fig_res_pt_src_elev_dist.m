...
for i0 = 1:length(phant_id_vec)
	p0 = phant_id_vec(i0);
	[phant_sys_ab_pos, phant_sys_c_pos] = computeElevationStatistics(...
...
for i0 = 1:length(vivo_id_vec)
	v0 = vivo_id_vec(i0);
	[vivo_sys_ab_pos, vivo_sys_c_pos] = computeElevationStatistics(...
		vivo_sys_ab_struct(v0), vivo_sys_c_struct(v0), vivo_time_mat(i0, :), ...
		'ptSrcAxialLimits', vivo_pt_src_ax_limit_mat(i0, :));

	if i0 <= 2
		% Add a negative sign to the DetectionNet mean, because apparently we rotated the probe by 180 degrees between trials.
...
output_img_file_name = fullfile(output_img_dir_name, 'res_pt_src_elev_dist.png');
