...
function makeVideo(trial_id, t_start, t_end, varargin)
	...
	sys_ab_time = h5read(robot_poses_file_name, '/sys_ab/time');
	...
	while live_vh.CurrentTime < live_vid_end
		...
		z_c = find(sys_c_time - t_start <= live_vid_t_curr);

		if isempty(z_c)
			% Off-by-one correction.
			z_c = chdat_start_id + 1;
		end

		% Channel data file names start from zero.
		i_c = z_c(end) - 1;
		[~, i_ab] = min(abs(sys_ab_time - sys_c_time(z_c(end))));
		i_ab = i_ab - 1;

		input_chdat_file_name = fullfile(input_dir_name, sprintf('chdat_%06d.h5', i_c));
		fprintf(['Reading file ''', input_chdat_file_name, '''...\n']);
		ch_data = permute(h5read(input_chdat_file_name, '/chdat'), [3, 2, 1]);

		output_das_pa_file_name = fullfile(input_dir_name, sprintf('das_pa_%06d.mat', i_c));
		...
		input_seg_mask_file_name = fullfile(input_dir_name, sprintf('seg_mask_c_%06d.h5', i_c));
		...
		input_sys_c_file_name = fullfile(input_dir_name, sprintf('sys_c_%06d.h5', i_c));
		input_sys_ab_file_name = fullfile(input_dir_name, sprintf('sys_ab_%06d.h5', i_ab));

		if exist(input_sys_c_file_name, 'file')
			fprintf(['Reading file ''', input_sys_c_file_name, '''...\n']);
			...
			fprintf(['Reading file ''', input_sys_ab_file_name, '''...\n']);
			sys_ab_loc_est = h5read(input_sys_ab_file_name, '/pt_src/p42v_link1');

			hold on
			plot(...
				1e3 .* sys_c_loc_est(1), 1e3 .* sys_c_loc_est(3) * 1540.0 / sys_c_sos_est, ...
				'LineStyle', 'none', 'Marker', '+', 'MarkerSize', 15, 'DisplayName', 'WaveSegNet-2');
			plot(...
				1e3 .* sys_ab_loc_est(1), 1e3 .* sys_ab_loc_est(3), 'Marker', '+', 'MarkerSize', 15, ...
				'LineStyle', 'none', 'DisplayName', 'WaveSegNet-1');
			hold off
		end
		...
		ylabel('Axial [mm]');
		title('Delay-and-Sum');
		legend('location', 'northeast', 'Direction', 'reverse');
