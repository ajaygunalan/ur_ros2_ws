 ...
 */
GubbiVisualServoingClass::GubbiVisualServoingClass(ros::NodeHandle& nh)
  : nh(nh)
  ...
  , guidance_lkf_track_id(-1)
{
  ...
  {
    ROS_WARN("Could not find ROS parameter 'lkf_track_dist_filter_coeff', using default value...");
  ...
  {
    ROS_WARN("Could not fetch parameter 'detection_topic_name', using default value...");
  ...
  {
    ROS_WARN("Could not fetch parameter 'force_torque_topic_name', using default value...");
  ...
  // TODO: Initialize remaining subscribers and timers.
}
...
void GubbiVisualServoingClass::detectionSubscriberCallback(pulse_vs_msgs::Segmentation2DArray det_msg)
{
  ...
  try
  {
    ...
    ROS_DEBUG("Extracting source positions from detection message...");
    ...
    // TODO: Set a flag to reset the controller if need be.
  }
